    ...
    const obtenerCatalogo = async () => {
      ...
      try {
        ...
        if (respuesta.ok) {
          ...
          mostrarAlerta("Error", datos.detail || "Error al cargar el catálogo.");
        }
      } catch (error: any) {
    ...
    return (
      <View style={styles.contenedorFondo}>
        <ScrollView style={styles.contenedorPantalla} contentContainerStyle={styles.scrollContenido}>
          <Text style={styles.textoNoProductos}>No hay productos disponibles en el catálogo.</Text>
        </ScrollView>
      </View>
    );
  }
          ...
          return (
            <View key={index} style={styles.contenedorFilaCategoria}>
              ...
              <View style={styles.filaTitulo}>
                <Text style={styles.tituloCategoria}>{categoria.toUpperCase()}</Text>
                <Pressable onPress={() => toggleExpandir(categoria)}>
                  <Text style={styles.textoVerTodo}>{estaExpandido ? 'Ver menos' : 'Ver todo'}</Text>
                </Pressable>
              </View>

              {estaExpandido ? (
                <View style={[styles.gridExpandido, esMovil && styles.gridExpandidoMovil]}>
                ...
                !esMovil ? (
                  <View style={styles.filaArticulosPcWrapper}>
                    <View style={styles.contenedorFlecha}>
                      {indicesCatalogo[categoria] > 0 && (
                        <Pressable onPress={() => paginaAnterior(categoria)}>
                          <Image source={require('@/assets/images/iconoFlechaIzq.png')} style={styles.iconoFlecha} resizeMode="contain" />
                        </Pressable>
                      )}
                    </View>
                    ...
                    <View style={styles.contenedorFlecha}>
                      {indicesCatalogo[categoria] + 4 < articulos.length && (
                        <Pressable onPress={() => paginaSiguiente(categoria, articulos.length)}>
                          <Image source={require('@/assets/images/iconoFlechaDer.png')} style={styles.iconoFlecha} resizeMode="contain" />
                        </Pressable>
                      )}
                    </View>
                  </View>
                  ...
                  <FlatList 
                    ...
                    renderItem={({ item, index: i }) => (
                      <Link href={{ pathname: '/vista-articulo', params: { id: item.id } }} asChild>
                        <Pressable style={{ marginRight: i === articulos.length - 1 ? 0 : 15 }}>
                          <TarjetaArticulo articulo={item} anchoTarjeta={anchoTarjetaMovil} />
                        </Pressable>
                      </Link>
                    )}
